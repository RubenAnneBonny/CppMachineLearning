...
void equality_operator_test(){
    LinAlg::Tensor<int> A {{2, 3}};
    LinAlg::Tensor<int> B {{2, 3}};
    LinAlg::Tensor<int> C {{2, 3}};
    LinAlg::Tensor<int> D {{1, 2, 3}};
    LinAlg::Tensor<int> E {{3, 3}};
...
void matrix_multiplication_test(){
    LinAlg::Tensor<int> A {{2, 3}, 1};
    LinAlg::Tensor<int> B {{3, 1}, 1};
    ...
    LinAlg::Tensor<int> C {A * B};
    LinAlg::Tensor<int> C_exp {{2, 1}, -5};
...
void pairwise_multiplication_test(){
    ...
    LinAlg::Tensor<int> C {LinAlg::pairwise_mult(A, B)};
    LinAlg::Tensor<int> C_exp {{2, 2, 3}, 1};
...
void elementwise_multiplication_test(){
    ...
    LinAlg::Tensor<int> C {(A * 3)};
    LinAlg::Tensor<int> C_exp {{2, 2, 3}, 3};
...
void pairwise_addition_test() {
    ...
    LinAlg::Tensor<int> C{A + B};
    LinAlg::Tensor<int> C_exp{{2, 2, 3}, 2};
...
void elementwise_addition_test() {
    ...
    LinAlg::Tensor<int> B {A + 6};
    LinAlg::Tensor<int> B_exp {{2, 2, 3}, 7};

    B_exp({0, 0, 1}) = 8;
    B_exp({0, 1, 2}) = 9;
    B_exp({1, 1, 0}) = 8;

    assert(B == B_exp && "Elementwise addition between tensor and int fails");
}

void tensor_merging_test() {
    LinAlg::Tensor<int> A({1, 2}, 1);
    LinAlg::Tensor<int> B({2, 2}, 1);

    A({0, 0}) = -3;
    B({1, 1}) = 2;

    std::vector<LinAlg::Tensor<int>> tensors {A, B};

    LinAlg::Tensor<int> C {tensors, 0};
    LinAlg::Tensor<int> C_exp {{3, 2}, 1};

    C_exp({0, 0}) = -3;
    C_exp({2, 1}) = 2;

    assert(C == C_exp && "Tensor merging does not work");
}

void unsqueeze_test() {
    LinAlg::Tensor<int> A {{2, 2}, 1};

    A({1, 1}) = 3;

    A.unsqueeze(1);

    LinAlg::Tensor<int> A_exp {{2, 1, 2}, 1};
...
void squeeze_test() {
    LinAlg::Tensor<int> A {{2, 1, 2}, 1};

    A({0, 0, 0}) = -5;

    A.squeeze(1);

    LinAlg::Tensor<int> A_exp ({2, 2}, 1);
...
void t_test() {
    ...
    LinAlg::Tensor<int> B {A.t()};
    LinAlg::Tensor<int> B_exp {{2, 3, 2}, 1};
