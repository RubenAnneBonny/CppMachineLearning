...
namespace Data {
    ...
    Data_set<T> make_circles(Rand::Random<T>& random, int num_samples, T noise = T{0.05}, T radius_factor = T{0.5}) {
        ...
        for(int i {}; i < num_samples; ++i) {
            ...
            X[{i, 0}] = r * std::cos(theta) + random.normal(T{0}, noise);
            X[{i, 1}] = r * std::sin(theta) + random.normal(T{0}, noise);
            
            Y[{i, cls}] = T{1};
        }

        return Data_set<T>{X, Y};
    }
    ...
    Data_set<T> make_moons(Rand::Random<T>& random, int num_samples, T noise = T{0.1}) {
        ...
        for(int i {}; i < num_samples; ++i) {
            ...
            if(cls == 0) {
                ...
                X[{i, 0}] = T{1} - std::cos(theta);
                X[{i, 1}] = T{0.5} - std::sin(theta);
            }
    ...
    Data_set<T> make_spirals(Rand::Random<T>& random, int num_samples_per_class, int classes = 3, T noise = T{0.2}) {
        ...
        for(int cls {}; cls < classes; ++cls) {
            for(int i {}; i < samples_per_class; ++i, ++cls) {
                T fraction {static_cast<T>(j) / static_cast<T>(per_class)};
                T radius = fraction;
                T theta = frac * T{3} + (T{2} * std::numbers::pi_v<T> * static_cast<T>(cls)) / static_cast<T>(classes) + random.normal(T{0}, noise);
    ...
    T black_scholes_call(T S, T K, T time_to_expiry, T sigma, T r) {
        ...
        T inv_sqrt_2 {static_cast<T>(0.77071067811865475244)};
        auto norm_cdf{
            [inv_sqrt_2](T x)
            {
    ...
    Data_set<T> make_black_scholes(Rand::Random<T>& random, int num_samples) {
        ...
        return Data_set<T>{inputs, targets};
    }
}
